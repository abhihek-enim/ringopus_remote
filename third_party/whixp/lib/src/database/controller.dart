...
class DatabaseController {
  ...
  Future<void> initialize({int maxRetries = 5}) async {
    ...
    if (_isInitializing) {
      ...
      // Ringopus fix: the lock only means the *attempt* finished, not that
      // it succeeded - re-check so a failed initializer doesn't leave a
      // concurrent caller (e.g. Session.resume() reading SM state right
      // after the stream connects) thinking the database is ready.
      _ensureInitialized();
      return;
    }
    ...
    // Ringopus fix: _initLock used to complete() immediately here, before
    // any of the real init work below had run. Any concurrent caller that
    // saw _isInitializing == true would await the lock, wake up right away,
    // and return believing the database was open - while _database was
    // still null and _initialized still false. Completing the lock in
    // `finally` instead means waiters only wake once this attempt has
    // truly finished, success or failure.
    try {
