...
void FlutterScreenCapture::GetDisplayMedia(
  ...
  // the sharer's cursor can be excluded from the captured frame. Stock
  // flutter_webrtc always calls CreateDesktopCapturer(source) → showCursor
  // defaults to true. Only effective for SCREEN capture; libwebrtc always
  // composites the cursor for window capture. See DECISIONS.md.
  ...
  if (video != EncodableMap()) {
    ...
    if (findString(video, "cursor") == "never") {
      show_cursor = false;
    }
  }
