...
import 'package:ringopus_remote_producer/mediasoup/src/consumer.dart';
import 'package:ringopus_remote_producer/mediasoup/src/data_consumer.dart';
import 'package:ringopus_remote_producer/mediasoup/src/data_producer.dart';
import 'package:ringopus_remote_producer/mediasoup/src/FlexQueue/flex_queue.dart';
import 'package:ringopus_remote_producer/mediasoup/src/ortc.dart';
import 'package:ringopus_remote_producer/mediasoup/src/producer.dart';
import 'package:ringopus_remote_producer/mediasoup/src/rtp_parameters.dart';
import 'package:ringopus_remote_producer/mediasoup/src/sctp_parameters.dart';
import 'package:ringopus_remote_producer/mediasoup/src/common/enhanced_event_emitter.dart';
import 'package:ringopus_remote_producer/mediasoup/src/common/logger.dart';
import 'package:ringopus_remote_producer/mediasoup/src/common/index.dart';
import 'package:ringopus_remote_producer/mediasoup/src/handlers/handler_interface.dart';
  ...
  Map<String, dynamic> toMap() {
    return {
      ...
      'fingerprints': fingerprints
          .map((DtlsFingerprint fp) => fp.toMap())
          .toList(),
    };
  }
}
...
class Transport extends EnhancedEventEmitter {
  ...
  // Resolves once _handler.run() has finished setting up the underlying
  // RTCPeerConnection. Must be awaited before produce()/consume() - see
  // handler_interface.dart's `run` doc comment for why this exists.
  late Future<void> _handlerReady;
  ...
  void _handleHandler() {
    ...
    handler.on('@connectionstatechange', (Map data) {
  ...
  /// Resolves once the underlying RTCPeerConnection is actually ready.
  /// Await this before calling produce()/consume() - see the note on
  /// `_handlerReady`.
  Future<void> get handlerReady => _handlerReady;
  ...
  Future<void> _produce(ProduceArguments arguments) async {
    try {
      ...
      if (arguments.encodings.isEmpty) {
        ...
        normalizedEncodings = arguments.encodings.map((
          RtpEncodingParameters encoding,
        ) {
          RtpEncodingParameters normalizedEncoding = RtpEncodingParameters(
            active: true,
          );
  ...
  void produce({
    ...
    if (_direction != Direction.send) {
      throw ('not a sending Transport');
        ...
        !_canProduceByKind.canIt(
          RTCRtpMediaTypeExtension.fromString(track.kind!),
        )) {
  ...
  Future<void> _consume(ConsumeArguments arguments) async {
    ...
    bool canConsume = Ortc.canReceive(
      arguments.rtpParameters,
  ...
  void produceData({
    ...
    _flexQueue.addTask(
      FlexTaskAdd(
        id: '',
        execFun: () async {
          HandlerSendDataChannelResult sendDataResult = await _handler
              .sendDataChannel(
                SendDataChannelArguments(
                  ordered: ordered,
