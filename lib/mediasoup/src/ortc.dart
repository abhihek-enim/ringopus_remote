...
class Ortc {
  ...
  static void validateAndNormalizeRtpCodecParameters(
    ...
    final RegExp mimeTypeRegex = RegExp(
      r"^(audio|video)/(.+)",
      caseSensitive: true,
    );
    ...
    final Iterable<RegExpMatch> mimeTypeMatch = mimeTypeRegex.allMatches(
      codec.mimeType,
    );
  ...
  static ExtendedRtpCapabilities getExtendedRtpCapabilities(
    ...
    for (RtpCodecCapability remoteCodec in remoteCaps.codecs) {
      ...
      final RtpCodecCapability? matchingLocalCodec = localCaps.codecs
          .firstWhereOrNull(
    ...
    for (ExtendedRtpCodec extendedCodec in extendedRtpCapabilities.codecs) {
      RtpCodecCapability? matchingLocalRtxCodec = localCaps.codecs
          .firstWhereOrNull(
      ...
      final RtpCodecCapability? matchingRemoteRtxCodec = remoteCaps.codecs
          .firstWhereOrNull(
    ...
    for (RtpHeaderExtension remoteExt in remoteCaps.headerExtensions) {
      final RtpHeaderExtension? matchingLocalExt = localCaps.headerExtensions
          .firstWhereOrNull(
  ...
  static List<RtpCodecParameters> reduceCodecs(
    ...
    // Helper function to find RTX codec for a given main codec by apt value
    RtpCodecParameters? findRtxForCodec(RtpCodecParameters mainCodec) {
      for (RtpCodecParameters codec in codecs) {
        if (isRtxCodec(codec)) {
          int? apt = codec.parameters['apt'];
          if (apt != null && apt == mainCodec.payloadType) {
            return codec;
          }
        }
      }
    ...
    // if no capability codec is given, take the first one (and its RTX).
    if (capCodec == null) {
      filteredCodecs.add(codecs.first);

      RtpCodecParameters? rtxCodec = findRtxForCodec(codecs.first);
      if (rtxCodec != null) {
        filteredCodecs.add(rtxCodec);
      }
    } else {
      ...
      for (int idx = 0; idx < codecs.length; ++idx) {
        if (matchCodecs(aCodec: codecs[idx], bCodec: capCodec)) {
          ...
          // Find RTX codec by apt value, not by position
          RtpCodecParameters? rtxCodec = findRtxForCodec(codecs[idx]);
          if (rtxCodec != null) {
            filteredCodecs.add(rtxCodec);
          }
  ...
  static bool canSend(
    ...
    return extendedRtpCapabilities.codecs.any(
      (ExtendedRtpCodec codec) => codec.kind == kind,
    );
  }
