...
import 'package:ringopus_remote_producer/mediasoup/src/handlers/unified_plan.dart';
...
class RTCOAuthCredential {
  ...
  const RTCOAuthCredential({required this.accessToken, required this.macKey});
}
...
class ReplaceTrackOptions {
  ...
  const ReplaceTrackOptions({required this.localId, required this.track});
}
...
abstract class HandlerInterface extends EnhancedEventEmitter {
  ...
  // Patched from the upstream `void run(...) async` signature: as `void`,
  // its Future was unobservable from Transport's (necessarily synchronous)
  // constructor, so `send()`/`receive()` could run before the underlying
  // RTCPeerConnection existed - see Transport._handlerReady.
  Future<void> run({required HandlerRunOptions options});
