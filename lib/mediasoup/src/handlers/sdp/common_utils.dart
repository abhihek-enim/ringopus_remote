...
class CommonUtils {
  static RtpCapabilities extractRtpCapabilities(SdpObject sdpObject) {
    ...
    for (MediaObject m in sdpObject.media) {
      ...
      for (Ext ext in m.ext ?? []) {
        ...
        // Key by kind+URI: the same URI legitimately appears in BOTH the
        // audio and video sections (e.g. abs-send-time, transport-wide-cc,
        // sdes:mid). Keying by URI alone made audio (parsed first) swallow
        // the shared URIs, so video capabilities lacked transport-wide-cc —
        // the ORTC matcher requires a kind+URI match, so video producers
        // negotiated without TWCC and libwebrtc's delay-based bandwidth
        // estimation was disabled for video. Upstream mediasoup-client
        // (commonUtils.ts) keeps one entry per kind+URI.
        String? uri = ext.uri;
        String mapKey = '$kind|$uri';
        if (uri != null && !headerExtensionsMap.containsKey(mapKey)) {
          ...
          headerExtensionsMap[mapKey] = headerExtension;
        }
      }
    }
        ...
        if (rtp.codec.toLowerCase() == 'h264' ||
          ...
          if (sdpFmtp != null) {
            ...
            if (profileMatches && rtp.codec.toLowerCase() == 'h264') {
              ...
              String? codecMode = codec.parameters['packetization-mode']
                  ?.toString();
                ...
                if (rtpCodec.mimeType.toLowerCase() == targetMime &&
                  ...
                  if (targetMime == 'video/vp9') {
                    String? templateProfile = templateCodec
                        ...
                        ?.toString();
                    String? rtpProfile = rtpCodec.parameters['profile-id']
                        ?.toString();
                    ...
                    String? templateProfile = templateCodec
                        ...
                        ?.toString();
                    String? rtpProfile = rtpCodec.parameters['profile-level-id']
                        ?.toString();
                    if (templateProfile != null &&
                        rtpProfile != null &&
  ...
  static void applyCodecParameters(
    ...
    for (RtpCodecParameters codec in offerRtpParameters.codecs) {
      ...
      Map<dynamic, dynamic> parameters = fmtp != null
          ? parseParams(fmtp.config)
          : <dynamic, dynamic>{};
      switch (mimeType) {
        case 'audio/opus':
          {
