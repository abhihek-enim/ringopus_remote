...
- `lib/mediasoup_signaling.dart` — orchestrates the vendored `lib/mediasoup/` client against the JSON-over-XMPP signaling protocol (transport creation, produce/consume, connect acks). On produce it applies the latency tuning: bitrate cap, explicit 30fps, degradation preference, and encoder-side downscale (`scaleResolutionDownBy` — the desktop capturer ignores width/height constraints entirely, so this is the only working resolution knob). Also owns the customer half of hold/transfer: `pauseSending()`/`resumeSending()` and `rebindDataConsumer()` (partial teardown — swaps only the input leg on agent transfer). See DECISIONS.md for all of it.
...
**Hold & transfer: this app is purely reactive.** Agents (in `ringopus_user_app`) can hold/resume a session or transfer it to another agent; the customer never initiates any of it. This app handles `session-held`/`session-resumed`/`session-agent-changed` plus a re-sent `data-consumer-params`, keeps `_Phase.sharing` throughout (hold is the orthogonal `_agentOnHold` flag, not a phase), and guards `session-incoming` against firing mid-session. The server/agent halves live in `ringopus_user_app` (`server/`, `useRemoteSession.ts`) — protocol changes must be coordinated across all three.
